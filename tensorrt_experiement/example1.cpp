...
/// Using unique_ptr with Destroy is optional, but beats calling destroy() for
/// everything Borrowed from the NVidia tutorial, nice C++ skills !
template <typename T>
struct Destroy {
...
nvinfer1::ICudaEngine *createCudaEngine(const std::string &onnxFileName,
  ...
  unique_ptr<INetworkDefinition, Destroy<INetworkDefinition>> network{
      builder->createNetworkV2(
          1U << (unsigned)NetworkDefinitionCreationFlag::kEXPLICIT_BATCH)};
  unique_ptr<nvonnxparser::IParser, Destroy<nvonnxparser::IParser>> parser{
      nvonnxparser::createParser(*network, logger)};

  if (!parser->parseFromFile(onnxFileName.c_str(),
...
void launchInference(nvinfer1::IExecutionContext *context, cudaStream_t stream,
                     std::vector<float> const &inputTensor,
                     std::vector<float> &outputTensor, void **bindings,
                     int batchSize) {
  int inputId = 0,
      outputId = 1;  // Here I assume input=0, output=1 for the current network

  // Infer synchronously as an alternative, no stream needed
  //    cudaMemcpy(bindings[inputId], inputTensor.data(), inputTensor.size() *
  //    sizeof(float), cudaMemcpyHostToDevice); bool res =
  //    context->executeV2(bindings); cudaMemcpy(outputTensor.data(),
  //    bindings[outputId], outputTensor.size() * sizeof(float),
  //    cudaMemcpyDeviceToHost);

  // Infer asynchronously, in a proper cuda way !
  cudaMemcpyAsync(bindings[inputId], inputTensor.data(),
                  inputTensor.size() * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  context->enqueueV2(bindings, stream, nullptr);
  cudaMemcpyAsync(outputTensor.data(), bindings[outputId],
                  outputTensor.size() * sizeof(float), cudaMemcpyDeviceToHost,
                  stream);
}
...
int main() {
  ...
  for (int i = 0; i < engine->getNbBindings(); ++i) {
    ...
    size_t size = accumulate(dims.d, dims.d + dims.nbDims, batchSize,
                             multiplies<size_t>());
  ...
  launchInference(context.get(), stream, inputTensor, outputTensor, bindings,
                  batchSize);
